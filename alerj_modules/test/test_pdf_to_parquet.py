...
def test_alerj_pdf_to_parquet():
    ...
    pdf_path = alerj_download_file(2023, 2)
    
    parquet_path = alerj_pdf_to_parquet(pdf_path)

    col_names = ["nome", "cargo", "funcao",	"rendimento_funcionario","comissao",
                 "bonificacao","incorporado","trienio","abono_de_permanencia","ferias",
                 "redutor","ipalerj_mens","pensao_alimenticia","previdencia","imposto_de_renda",
                 "indenizatoria","rendimento_liquido"]
    
    df_pdf = pd.read_parquet(parquet_path)
    print(df_pdf.columns)
